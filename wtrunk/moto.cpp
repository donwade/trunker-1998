...
void
motoSystem::periodic(BOOL force)
{
		...
		if (mySys)
		{
			...
			for (i = 0; i < 8; ++i)
			{
				...
				if ( mySys->bankActive[i] == 'y' )
					QuickOut(STATROW + 1, 19 + i, color_norm, tt);
				else if (mySys->bankPresent(i))
				{
					...
					QuickOut(STATROW + 1, 19 + i, color_quiet, tt);
			}
		}
		stats(scanTypeEnv);
		winCtr = now;
...
void
motoSystem::noteIdentifier(unsigned short possibleId, BOOL isOld)
{
...
void
motoSystem::twoOSWcall(unsigned short flags)
{
/*
 ...
 */
	...
	if ( NOW.grp )
	{
		blockNum = (unsigned short)((NOW.id >> 13) & 7);
		...
		if (isTypeII(banktype))
		{
			...
			mySys->type2Gcall(
			    NOW.cmd,                                                               // frequency
			    (unsigned short)(NOW.id & 0xfff0),                                     // group ID
			    PAST.id,                                                                // calling ID
			    (unsigned short)((NOW.id & 0x000f) | flags)                            // flags
			    );
		...
		else if (isTypeI(banktype))
		{
			mySys->typeIGcall(
			    NOW.cmd,                                                               // frequency
			    banktype,
			    typeIgroup(NOW.id, banktype),                                          // group ID
			    typeIradio(NOW.id, banktype)                                           // calling ID
			    );
		}
		else if (isHybrid(banktype))
		{
			mySys->hybridGcall(
			    ...
			    typeIgroup(NOW.id, banktype),                                              // group ID
			    PAST.id,                                                                    // true ID
			    typeIradio(NOW.id, banktype)                                               // alias ID
			    );
		}
		else
		{
...
void
motoSystem::show_good_osw(register struct osw_stru* Inposw)
{
	unsigned short blockNum;
	char banktype;
	...
	if (PAST.cmd == OSW_FIRST_NORMAL || PAST.cmd == OSW_FIRST_TY2AS1)
	{
		switch (NOW.cmd)
		{
		case OSW_EXTENDED_FCN:

			if (NOW.id & 0xfff0 == 0x26f0)                                      // ack type 2 text msg
			{
				if (mySys && Verbose)
					mySys->note_text_msg(PAST.id, "Msg", NOW.id & ((unsigned short)15));
			}
			else if (NOW.id & 0xfff8 == 0x26E0)                                        // ack type 2 status
			{
				if (mySys && Verbose)
					mySys->note_text_msg(PAST.id, "Status", NOW.id & ((unsigned short)7));
			}
			else if ( FUTURE.isFreq && (FUTURE.id & 0xff00) == 0x1F00 &&
			          (NOW.id & 0xfc00) == 0x2800 && FUTURE.cmd == (NOW.id & 0x3FF))
			{
				/* non-smartzone identity:  sysid[10]
				   ...
				   1Fxx                 x   <ffffffffff>
				 */
				noteIdentifier(PAST.id, FALSE);
				if ( mySys )
					mySys->noteDataChan(FUTURE.cmd);
				numConsumed = 3;                                                 // we used up all 3
				return;
			}
			else if ((NOW.id & 0xfc00) == 0x2800)
			{
				...
				noteIdentifier(PAST.id, FALSE);
				if ( mySys )
					mySys->noteDataChan((unsigned short)(NOW.id & 0x03ff));
			}
			else if ((NOW.id & 0xfc00) == 0x6000)
			{
			...
			else if ((NOW.id & 0xfc00) == 0x2400)                             // various
			{
				idlesWithNoId = 0;
				if ( (NOW.id & 0x03ff) == 0x21c)
					if ( mySys && Verbose)
						mySys->note_affiliation(PAST.id, 0);
			}
			else if ( NOW.id  == 0x2021)
			{
				if (mySys)
					mySys->mscancel(PAST.id);
		...
		case OSW_NET_INFO:
			if (FUTURE.cmd == OSW_EXTENDED_FCN)
			{
				...
				numConsumed = 3;
				idlesWithNoId = 0;                                                 // got identifying info...
				if ( (FUTURE.id & 0xfc00) == 0x6000)
				{
					if (mySys && (PAST.id == mySys->sysId) )
					{
						mySys->note_neighbor((unsigned short)((NOW.id >> 10) & 0x3F), (unsigned short)(FUTURE.id & 0x3ff));
						if ( mySys->isNetworkable() && !mySys->isNetworked())
							mySys->setNetworked();
						...
						if (mySys->isNetworked() && ((unsigned short)((NOW.id >> 10) & 0x3F) ==  mySys->siteId) )
						{
							if (FUTURE.grp)                                                                                      // if active data channel
							{
								if ( NOW.id & 8 )
								...
								mySys->noteAltDchan((unsigned short)(FUTURE.id & 0x3ff));
						}
					}
				}
				return;
		...
		case OSW_TY2_AFFILIATION:
			...
			// whatever that means.
			if ( mySys && Verbose)
				mySys->note_affiliation(PAST.id, NOW.id);
		...
		case OSW_TY2_MESSAGE:
			...
			if ( mySys && Verbose)
			{
				mySys->note_text_msg(   PAST.id,
				                        "Msg",
				                        (unsigned short)(NOW.id & 7)
				                        );
			}
			break;
		...
		case OSW_TY2_CALL_ALERT:
			...
			if ( mySys && Verbose)
				mySys->note_page(PAST.id, NOW.id);
			break;
		...
		case OSW_SYSTEM_CLOCK:                  // system clock
			if ( mySys)
			{
				...
				if ( tt2 != ott2 || tt1 != ott1 )
				{
					...
					unsigned minutes = (tt2)   & 0x00ff;
					unsigned hours = (tt2 >> 8) & 0x001f;
					
					unsigned month = (tt1 >> 5) & 0x000f;
					unsigned days = (tt1)   & 0x001f;
					unsigned year = (tt1 >> 9);
		...
		case OSW_EMERG_ANNC:
			if (mySys)
				mySys->noteEmergencySignal(PAST.id);
			break;
		case OSW_AFFIL_FCN:
			if ( mySys )
			{
				switch (NOW.id & 0x000f)
				{
				case 0:
				case 1:
				case 2:
		...
		case 0x34a:                 // size code K
			...
			{
				// patch notification
				// patch 'tag' is in NOW.id
				//group associated is PAST.id
				...
				case 4:                                                     // emergency patch
					...
					mySys->note_patch(/*tag*/ NOW.id, /*group*/ PAST.id,
					                          (char)('A' + (NOW.cmd - 0x340)) );
					break;
				case 5:
				case 7:
					mySys->note_patch(/*tag*/ NOW.id, /*group*/ PAST.id,
					                          (char)('A' + (NOW.cmd - 0x340)), TRUE);
					break;
				default:
					break;
				}
		...
		case 0x34c:                 // size code M trunker = L
			if ( mySys )
			{
				...
				case 4:                                                     // emergency patch
					...
					mySys->note_patch(/*tag*/ NOW.id, /*group*/ PAST.id, 'L');
					break;
				case 5:
				case 7:
					mySys->note_patch(/*tag*/ NOW.id, /*group*/ PAST.id,
		...
		case 0x34e:                 // size code O Trunker = M
			if ( mySys )
			{
				...
				case 4:                                                     // emergency patch
					...
					mySys->note_patch(/*tag*/ NOW.id, /*group*/ PAST.id, 'M');
					break;
				case 5:
				case 7:
					mySys->note_patch(/*tag*/ NOW.id, /*group*/ PAST.id,
		...
		case 0x350:                 // size code Q Trunker = N
			...
			if ( mySys )
			{
				...
				case 4:                                                     // emergency patch
					...
					mySys->note_patch(/*tag*/ NOW.id, /*group*/ PAST.id, 'N');
					break;
				case 5:
				case 7:
					mySys->note_patch(/*tag*/ NOW.id, /*group*/ PAST.id,
		...
		default:
			...
			if ( mySys )
			{
				if (NOW.isFreq)
				{
					if ( PAST.cmd == OSW_FIRST_TY2AS1 && NOW.grp)
					{
						blockNum = (unsigned short)((NOW.id >> 13) & 7);
						banktype = mySys->types[blockNum];
						if ( banktype == '2' )
					...
					if ( PAST.id != 0 )                                         // zero id used special way in hybrid system
						twoOSWcall();
				}
			}
	...
	if ( NOW.cmd == 0x0320 && FUTURE.cmd == OSW_EXTENDED_FCN && PAST.isFreq)
	{
		numConsumed = 3;
		idlesWithNoId = 0;                         // got identifying info...
		if ( (FUTURE.id & 0xfc00) == 0x6000)
		{
			if (mySys && (PAST.id == mySys->sysId) )
			{
				if ( myFreqMap->isFrequency((unsigned short)(FUTURE.id & 0x3ff), mySys->getBandPlan()) )
					mySys->note_neighbor((unsigned short)((NOW.id >> 10) & 0x3F), (unsigned short)(FUTURE.id & 0x3ff));
				if ( mySys->isNetworkable() && !mySys->isNetworked())
					mySys->setNetworked();
	...
	if ( (PAST.cmd == OSW_FIRST_ASTRO || PAST.cmd == OSW_FIRST_CODED_PC) && NOW.isFreq)
	{
		if ( mySys )
		{
			if ( PAST.id )
			{
				if (PAST.cmd == OSW_FIRST_ASTRO)
					mySys->setSubType(" Astro");
	...
	case OSW_SYS_STATUS:             // system status
		if (mySys)
		{
			...
			statnum = (unsigned short)((PAST.id >> 13) & 7);                                     // high order 3 bits opcode
			if (statnum == 1)
			{
				if (PAST.id & ((unsigned short)0x1000))
					mySys->setBasicType("Type II");
				else
					mySys->setBasicType("Type I");
				statnum = (unsigned short)(PAST.id >> 5);
				statnum &= 7;
	...
	case OSW_TY1_EMERGENCY:                     // type 1 emergency signal
		if (mySys)
		{
			...
			{
				blockNum = (unsigned short)((PAST.id >> 13) & 7);
				banktype = mySys->types[blockNum];
				if ( isTypeI(banktype) || isHybrid(banktype))
					mySys->noteEmergencySignal(typeIradio(PAST.id, banktype));
				else
					mySys->noteEmergencySignal(PAST.id);
			}
		}
	...
	case OSW_TY1_ALERT:
		if (mySys)
		{
			...
			banktype = mySys->types[blockNum];
			if ( isTypeI(banktype) || isHybrid(banktype))
				mySys->note_page(typeIradio(PAST.id, banktype));
			else
				mySys->note_page(PAST.id);
		}
		break;
	...
	case OSW_CW_ID:             // this seems to catch the appropriate diagnostic. thanks, wayne!
		if (mySys)
		{
			if ( (PAST.id & 0xe000)  == 0xe000)
			{
				cwFreq = (unsigned short)(PAST.id & 0x3ff);
				mySys->noteCwId(cwFreq, PAST.id & 0x1000);
			}
			else
			{
				static unsigned int lastDiag = 0;
				static time_t lastT = 0;
				if ( (PAST.id != lastDiag) || ((now - lastT) > 120))
				{
					switch (PAST.id & 0x0F00)
					{
					case 0x0A00:
						sprintf(tempArea, "Diag %04x: %s Enabled", PAST.id, getEquipName(PAST.id & 0x00ff));
						break;
					case 0x0B00:
						sprintf(tempArea, "Diag %04x: %s Disabled", PAST.id, getEquipName(PAST.id & 0x00ff));
						break;
					case 0x0C00:
						sprintf(tempArea, "Diag %04x: %s Malfunction", PAST.id, getEquipName(PAST.id & 0x00ff));
						break;
					default:
						sprintf(tempArea, "Diagnostic code (other): %04x", PAST.id);
					}
	...
	default:
		if (mySys)
		{
			if (PAST.cmd >= OSW_TY1_STATUS_MIN &&
			    PAST.cmd <= OSW_TY1_STATUS_MAX &&
			    numStacked > 4)
			{
				...
				if (mySys && Verbose)
				{
					mySys->note_text_msg(   PAST.id,
					                        "Status",
					                        (unsigned short)(PAST.cmd - OSW_TY1_STATUS_MIN)
					                        );
			...
			if (PAST.isFreq && (numStacked > 4)  )
			{
				if ( mySys->vhf_uhf &&                                                                  // we have such a system
				     NOW.isFreq &&                                                                 // this is a pair of frequencies
				     (mySys->mapInpFreq(PAST.cmd) == NOW.cmd)                                  // the first frequency is input to second
				     )
				{
					...
					if ( NOW.grp && !PAST.grp )                                                          // supposedly astro indication
						twoOSWcall(OTHER_DIGITAL);
					else
						twoOSWcall();
					numConsumed = 2;
				...
				else
				{
					...
					if (((PAST.id & 0xff00) == 0x1f00) && (osw_state == OperOlder))
					{
						noteIdentifier(PAST.id, TRUE);
						if ( mySys)
							mySys->noteDataChan(PAST.cmd);
					}
					else
					{
						...
						if ( PAST.grp)
						{
							...
							if (isTypeII(banktype))
							{
								mySys->type2Gcall(PAST.cmd,                                                                                                     // frequency
								                  (unsigned short)(PAST.id & 0xfff0),                                                                           // group ID
								                  (unsigned short)(PAST.id & 0x000f)                                                                            // flags
								                  );
							}
							else if (isTypeI(banktype))
							{
								mySys->typeIGcall(
								    PAST.cmd,                                                                                                                                       // frequency
								    banktype,
								    typeIgroup(PAST.id, banktype),                                                                                                                  // group ID
								    typeIradio(PAST.id, banktype)                                                                                                                   // calling ID
								    );
							}
							else if (isHybrid(banktype))
							{
		...
		{
			if (PAST.isFreq && ((PAST.id & 0xff00) == 0x1f00))
			{
				noteIdentifier(PAST.id, TRUE);
				noteIdentifier(PAST.id, TRUE);
			}
...
static int ct;
void
motoSystem::proc_osw(unsigned char sl)
{
	register int l;
...
//
void
motoSystem::pigout(int skipover)
{
	register int i1, i2;
...
mainLoop()
{
	...
	//
	mySys = new TrunkSys(0x1234, myScanner, "M");
